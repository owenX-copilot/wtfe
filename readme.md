...
wtfe/
├── core/                  # 核心数据模型
│   └── models.py         # FileFact, ModuleSummary, EntryPoint, RunConfig, ProjectContext
│
...
| 模块 | 状态 | 功能 |
|------|------|------|
| **core/models.py** | ✅ 完成 | 定义了所有数据结构（FileFact, ModuleSummary, EntryPoint, RunConfig, ProjectContext, Evidence, FileRole） |
...
4. 📋 创建统一 CLI 入口
5. 📋 实现 wtfe-readme（AI 生成层）

**当前状态**：已完成 Pipeline A1 和 B1，正在进入 Pipeline A2 开发阶段。
✅ ~~实现 wtfe-folder（Pipeline A2）~~
3. ✅ ~~实现 wtfe-context（Pipeline C）~~
4. 🚧 **创建统一 CLI 入口**
5. 📋 实现 wtfe-readme（AI 生成层）

**当前状态**：三条核心管道（A1/A2, B1, C）已全部完成，进入最终整合